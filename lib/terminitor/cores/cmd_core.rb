#
# ugly list
# - better way to find a window - currently waits for console to load 0.4s and then try to get hwnd once. need to dive
#   into win32 api for that although first signs show no hope.
# - break up into files and sort out module order.
# - remove unneeded windows/ refs that i've added as part of playing around
# - rename, clean up
# - find a heuristic/way to transpose the 'clear', 'open' methods for example into 'cls' and 'start. currently it works
#   for single word commands, however 'open' command comes usually with an argument, and i'd also don't like to do a 'starts with'
#   transpose... although i might have no choice. for now it remains non functional.
# - remove all puts debugs, into more proper logs.
#
#
...
module Terminitor
  ...
  class CmdCore < AbstractCore
    ...
    CMD_XLT = {
      'clear' => 'cls', #uncomment when debugging
      'open'  => 'start'
    }
  ...
  class WindowsConsole
    ...
    def start
      ...
      ret = WaitForInputIdle(@process_handle, 2000)
      @pid
    end
